...
.check_column_order_and_types <- function(df, expected_cols, expected_types) {
  ...
  if (any(invalid_cols)) {
    ...
    return(paste0(
      "The peaksData variable(s) ",
      paste(invalid_col_names, collapse = ", "),
...
.mzR_format_chromData <- function(file) {
  ...
  colnames(tmp)[
    colnames(tmp) == "precursorCollisionEnergy"
  ] <- "collisionEnergy"
  colnames(tmp)[
    colnames(tmp) == "productIsolationWindowTargetMZ"
  ] <- "productMz"
  colnames(tmp)[
    colnames(tmp) == "precursorIsolationWindowTargetMZ"
  ] <- "precursorMz"
  tmp$dataOrigin <- file
  tmp
}
...
#' @noRd
.plot_single_chromatogram <- function(
  x,
  xlab = "rtime (s)",
  ylab = "intensity",
  type = "l",
...
.ensure_rt_mz_columns <- function(chrom_data, spectra, spectra_f) {
  if (!all(c("mzMin", "mzMax") %in% colnames(chrom_data))) {
    if (
      "mzMin" %in% colnames(chrom_data) || "mzMax" %in% colnames(chrom_data)
    ) {
...
#' - `chromExtract()`.
#' @noRd
.validate_chromExtract_input <- function(
  object,
  peak.table,
  by,
  ...
  if (!all(required_cols %in% names(peak.table))) {
    stop(
      "`peak.table` must contain columns: ",
  ...
  if (anyNA(peak.table$rtMin) || anyNA(peak.table$rtMax)) {
    stop(
      "Columns 'rtMin' and 'rtMax' in `peak.table` cannot ",
      "contain NA values."
    )
  }
  ...
  if (nrow(cd) != nrow(unique(cd[, by, drop = FALSE]))) {
    stop(
      "Combinations of 'by' columns must uniquely identify rows ",
      "in chromData."
    )
  }
}
...
.match_chromdata_peaktable <- function(object, peak.table, by) {
  ...
  if (length(missing_keys)) {
    stop(
      "Some combinations in `peak.table` do not exist in chromData: ",
...
.check_overl_columns <- function(object, peak.table, required_cols) {
  ...
  if (length(extra_cols)) {
    warning(
      "The following columns in `peak.table` already exist in ",
...
.map_spectra_vars <- function(object, spectraVariables) {
  ...
  spectra <- .spectra(object) # the Spectra object from @spectra slot
  cd <- .chromData(object) # the chromData data.frame from @chromData slot
  ...
  if (any(spectraVariables %in% colnames(cd))) {
    ...
    non_replaceable <- vapply(
      existing,
      function(v) !all(is.na(cd[[v]])),
      logical(1)
    )
    if (any(non_replaceable)) {
      stop(
        "None of the 'spectraVariables' must already exist in the chromData data.frame."
      )
    }
  }
...
#' These directly access the internal slots (e.g., @chromData, @peaksData, @spectra, etc.).
