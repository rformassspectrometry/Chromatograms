...
chromatograms, following the same two-step design as `compareSpectra()` in the
`Spectra` package. First, a mapping function (`MAPFUN`, default `matchRtime()`)
aligns the two chromatograms onto a common retention-time grid by linear
interpolation. Then, a scoring function (`FUN`, default `cor()`, i.e. Pearson
correlation) computes a similarity value from the aligned intensity vectors.
Additional arguments such as `method = "spearman"` can be passed via `...`
to the scoring function, and a fully custom `FUN` or `MAPFUN` can be supplied.

## Comparing chromatograms within a single object

When called with a single `Chromatograms` object, `compareChromatograms()`
computes all pairwise similarities and returns a symmetric *n × n* matrix.
...
cor_mat <- compareChromatograms(chr_sub)
cor_mat

## Use a chromData column as row/column labels
cor_mat_labeled <- compareChromatograms(chr_sub, labels = "chromIndex")
cor_mat_labeled
```

The result is a symmetric matrix with 1 on the diagonal. We can visualise it
as a heatmap, labelling rows and columns with the MRM precursor → product m/z
transitions stored in `chromData()`:
...
rownames(cor_mat) <- colnames(cor_mat) <- mz_labels
pheatmap(cor_mat, main = "Pairwise Pearson correlation",
...
cos_mat <- compareChromatograms(chr_sub, FUN = cosine)
cos_mat
```
...
returns an *n × m* matrix with similarities between each chromatogram in `x`
(rows) and each in `y` (columns). When one side has length 1, the result is
simplified to a numeric vector by default (`SIMPLIFY = TRUE`).
...
## Compare a single chromatogram against all others → returns a vector
compareChromatograms(chr_sub[1], chr_sub[2:8])
```
