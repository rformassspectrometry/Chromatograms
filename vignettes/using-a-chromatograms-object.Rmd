...
# Comparing chromatograms with correlation

The `correlate()` function computes pairwise similarity between all
chromatograms in a `Chromatograms` object. Because two chromatograms rarely
share the exact same retention time values, intensities are first linearly
interpolated onto a common retention time grid before the similarity is
calculated. By default Pearson correlation is used, but Spearman and Kendall
are also available, and an arbitrary similarity function can be supplied via
`FUN`.
...
cor_mat <- correlate(chr_sub)
cor_mat

## Use a chromData column as row/column labels
cor_mat_labeled <- correlate(chr_sub, labels = "chromIndex")
cor_mat_labeled
```
...
```{r correlate-heatmap, fig.width=6, fig.height=5}
library(pheatmap)
...
cos_mat <- correlate(chr_sub, FUN = cosine)
cos_mat
```
