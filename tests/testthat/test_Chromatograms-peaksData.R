test_that("peaksData, Chromatograms, ChrombackendMemory works as expected", {
    ...
    expect_equal(rtime_data, peaksData(c_full@backend, columns = "rtime",
                                       drop = TRUE))
...
test_that("peaksData, Chromatogram, ChromBackendMzR works as expected", { #is that really necessary ?
    ...
    expect_type(rtime_data, "list")
    expect_equal(rtime_data, peaksData(c_mzr@backend, columns = "rtime",
                                       drop = TRUE))
...
test_that("peaksData replacement works as expected", {
    ...
    expect_error(peaksData(c_mzr) <- list(data.frame()),
                 "replace peaks data")
})
...
test_that("rtime accessor and replacement work as expected", {
    ...
    expect_error(rtime(c_mzr)[[1]] <- rep(1, length(rtime(c_mzr)[[1]])),
...
test_that("intensity accessor and replacement work as expected", {
    ...
    backend_intensity <- peaksData(c_full@backend, columns = "intensity",
                                   drop = TRUE)
    expect_equal(intensity_data, backend_intensity)

    intensity_data <- intensity(c_mzr)
    backend_intensity <- peaksData(c_mzr@backend, columns = "intensity",
                                   drop = TRUE)
    expect_equal(intensity_data, backend_intensity)
    ...
    expect_error(intensity(c_mzr)[[1]] <- rep(1, length(intensity(c_mzr)[[1]])),
                 "Cannot replace peaks data in a read-only backend")
})

test_that("filterPeaksData queues the correct processing step", {
    c_filtered <- filterPeaksData(c_full, variables = c("rtime"),
                                  ranges = c(12.5, 45.5))
